...
$apiRoot = Join-Path (Split-Path -Parent $root) 'PunchAPI'
$api = Join-Path $apiRoot 'build\PunchAPI.dll'
...
foreach ($required in @($Csc, $loader, $api, $unity, $mscorlib, $source)) {
...
& $Csc /nologo /target:library "/out:$output" "/reference:$unity" `
    "/reference:$loader" "/reference:$api" "/reference:$mscorlib" /nostdlib $source
